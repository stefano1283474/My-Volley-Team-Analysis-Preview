    ...
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      {activeSection === 'fnc' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          {/* Baselines table */}
          <div className="glass-card p-4 space-y-4">
            <h3 className="text-sm font-semibold text-gray-300">Baseline del campionato (dal dataset)</h3>
            ...
            <div className="text-[10px] text-gray-500 space-y-1 pt-2 border-t border-white/5">
              <p><span className="text-amber-400">Perché le scale sono diverse?</span></p>
              <p>
                Ricezione/Difesa: nella formula entrano sia R4 che R5 come positivi,
                gli errori (R1) sono rari → media intrinsecamente alta (~50-55%).
              </p>
              <p>
                Attacco/Battuta/Muro: conta solo il valore estremo (kill vs errore).
                La maggior parte delle azioni è neutra → media intrinsecamente bassa (10-30%).
              </p>
              <p className="text-amber-400/70">
                Senza FNC, un radar con Reception=40% e Attack=40% è fuorviante:
                Attack al 40% è eccellente, Reception al 40% è sotto la media.
              </p>
            </div>
          </div>
        </div>
      ...
      {activeSection === 'preview' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <div className="glass-card p-4 space-y-4">
            <h3 className="text-sm font-semibold text-gray-300">Radar team: grezzo vs. FNC</h3>
            {analytics?.matchAnalytics?.length > 0 && baselines ? (
              <FNCPreviewRadar analytics={analytics} baselines={baselines} fncConfig={fncConfig} />
            ) : (
              <p className="text-xs text-gray-500 italic">Carica partite per vedere la preview.</p>
            )}
          </div>
          <div className="glass-card p-4 space-y-4">
            <h3 className="text-sm font-semibold text-gray-300">Peso contesto per partita</h3>
            <WeightImpactChart analytics={analytics} />
            {!analytics?.matchAnalytics?.length && (
              <p className="text-xs text-gray-500 italic">Carica partite per vedere il grafico.</p>
            )}
          </div>
        </div>
      )}
